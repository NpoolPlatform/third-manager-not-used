  ...
  stages {
    ...
    stage('Release docker image for development') {
      ...
      steps {
        ...
        sh(returnStdout: false, script: '''
          images=`docker images | grep entropypool | grep third-manager | grep none | awk '{ print $3 }'`
    ...
    stage('Release docker image for testing') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          set +e
          docker images | grep third-manager | grep $tag
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
    ...
    stage('Deploy for development') {
      ...
      steps {
        sh 'sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/third-manager/k8s/02-third-manager.yaml'
    ...
    stage('Deploy for testing') {
      ...
      steps {
        sh(returnStdout: true, script: '''
          ...
          git reset --hard
          git checkout $tag
          sed -i "s/third-manager:latest/third-manager:$tag/g" cmd/third-manager/k8s/02-third-manager.yaml
          sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/third-manager/k8s/02-third-manager.yaml
